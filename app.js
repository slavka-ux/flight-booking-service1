/**
 * Flight Booking – клієнтська логіка з аутентифікацією
 */
...
class BookingApp {
    ...
    init() {
        ...
        document.getElementById('closeBooking').addEventListener('click', () => this.closeModal('booking'));
        window.addEventListener('click', (e) => {
            if (e.target.classList.contains('modal')) {
                this.closeModal(e.target.id.replace('Modal', '').toLowerCase());
            }
        });
    ...
    logout() {
        ...
        localStorage.removeItem('auth_token');
        this.updateUIForGuest();
        this.showMessage('Ви вийшли з системи', 'info');
    }
    ...
    async loadUserBookings() {
        ...
        try {
            const res = await fetch(`${this.apiBase}/api/bookings/user`, {
                headers: { 'Authorization': `Bearer ${this.token}` }
            });
            if (res.ok) {
                const data = await res.json();
                // Можна відобразити бронювання на сторінці
